...
run(fullfile('C:/matconvnet-1.0-beta16/matlab', 'vl_setupnn.m')) ;
...
%correcto? talvez innesecesario
labels = imdb.images.labels(1,batch) ;


% seed=randperm(length(batch),1);
% seq=randperm(length(batch),seed);
% im(:,:,:,seq)=flip(im(:,:,:,seq),2);
% im(:,:,:,seq) = flip(im(:,:,:,seq) ,1);
