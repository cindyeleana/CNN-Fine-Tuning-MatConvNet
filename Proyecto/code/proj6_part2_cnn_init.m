...
%urlwrite('http://www.cc.gatech.edu/~hays/compvision/proj6/imagenet-vgg-f.mat', 'imagenet-vgg-f.mat') 
...
net.layers{20} = struct('type', 'conv', ...
                           'weights', {{f*randn(1,1,4096,2, 'single'), zeros(1, 2, 'single')}}, ...%change last 8s to 15
                           'stride', 1, ...
                           'pad', 0, ...
                           'name', 'fc8') ;
...
net.layers{end+1}=net.layers{21};
net.layers{end+1}=net.layers{21};
net.layers{22}=net.layers{20};
...
vl_simplenn_display(net, 'inputSize', [227 227 3 50])
